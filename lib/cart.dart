...
class _CartState extends State<Cart> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: ListView(
        children: <Widget>[
Text("your cart have ${widget.cart.length} items", style: TextStyle(color: Colors.yellowAccent, fontSize: 20.0),),
...
Container(
  ...
  child: LiVi(context, widget.cart)),
        ],
      ),
    );
  }
  Widget LiVi(BuildContext context, List favourite){
   return ListView.builder(
        itemBuilder: (BuildContext _, int index)=> Card(
          ...
          child: Row(
            children: <Widget>[
              Image.network(widget.cart[index], height: 150.0, width: 200.0,),
              SizedBox(width: 30.0,),
              InkWell1(index,  context) ,
              SizedBox(width: 30.0,),
              InkWell2(index,  context, widget.cart[index]) ,
                          ],
                        ),
                        elevation: 2.0,
                      ),
                ...
                Widget InkWell2( int index,BuildContext context, String pic){
                  return InkWell(
                    ...
                    onTap: ()=> Navigator.push(context, MaterialPageRoute(builder: (_)=> ViewPage(pic))),
                  );
                }
}
      // onTap: (){
